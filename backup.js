  ...
  return (
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
      ...
      {citySuggestions.length > 0 && (
        <DropDownPicker
          items={citySuggestions.map(suggestion => ({ label: suggestion.value.city, value: suggestion.value }))}
          searchable={true}
          onChangeItem={item => handleCityPress(item.value)}
          containerStyle={{ width: '80%', maxHeight: 200, marginBottom: 20 }}
          zIndex={99999}
          placeholder="cities suggessions liste"
